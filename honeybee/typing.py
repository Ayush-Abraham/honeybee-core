...
def valid_string(value: str, input_name=""):
    ...
    try:
        ...
        raise TypeError(
            "Input {} must be a text string. Got {}: {}.".format(
...
def _number_check(value, input_name):
    ...
    try:
        ...
        raise TypeError(
            "Input {} must be a number. Got {}: {}.".format(
                input_name, type(value), value
            )
        )
    return number
...
def int_in_range(value, mi=INFNEG, ma=INFPOS, input_name=""):
    ...
    try:
        ...
        try:
            ...
            raise TypeError(
                "Input {} must be an integer. Got {}: {}.".format(
                    input_name, type(value), value
                )
            )
    except (ValueError, TypeError):
...
def tuple_with_length(value, length=3, item_type=float, input_name=""):
    ...
    try:
        ...
        raise TypeError("Input {} must be a {}.".format(input_name, item_type))
...
def readable_short_name(value, max_length=24):
    ...
    try:
        ...
        raise TypeError(
            "Input must be a text string. Got {}: {}.".format(type(value), value)
        )
    if len(value) <= max_length:
        return value
    ...
    assert mid_ind > 3, "Max character length of {} is too restrictive.".format(
        max_length
    )
...
def invalid_dict_error(invalid_dict, error):
    ...
    """
    obj_type = (
        invalid_dict["type"].replace("Abridged", "")
        if "type" in invalid_dict
        else "Honeybee Object"
    )
    obj_id = invalid_dict["identifier"] if "identifier" in invalid_dict else ""
    full_id = (
        "{}[{}]".format(invalid_dict["display_name"], obj_id)
        if "display_name" in invalid_dict
        else obj_id
    )
